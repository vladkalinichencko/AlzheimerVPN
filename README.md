...
- Unlike upstream, a connected tunnel can recover automatically after repeated DNS or traffic-health failures. The UI preserves the failure, shows `Recovering connection...`, and retries the tunnel up to three times; an explicit manual disconnect cancels recovery immediately.
- macOS wake recovery waits until the daemon sees a usable physical default route, keeps checking while that route is still unavailable, and drains queued route notifications before reconnecting. Waking the Mac no longer leaves recovery stalled until the user toggles the VPN manually.
- macOS no longer inherits the upstream IPv6 black hole: the daemon strips the synthetic IPv6 tunnel address and `::/0` routes, and tunnel startup requires the PF `block return` rule to be active. Native IPv6 cannot leak, while dual-stack services fall back to IPv4 instead of waiting on a dead tunnel path.
